...
(function () {
  ...
  function resizeFile(file, max = 1400) {
  ...
  function tidy(text) {
    return (text || "")
      ...
      .filter(l => l.length >= 3 && /[a-zA-ZáàâãéêíóôõúçÁÀÂÃÉÊÍÓÔÕÚÇ]/.test(l))
      .slice(0, 12)
      .join("\n");
  }
  ...
  function guessQuery(text) {
    ...
    return (core + (g ? " " + g + "g" : "")).trim();
  }

  window.runLocalOcr = async function (file, onProg) {
    const T = await loadEngine();
    const blob = await resizeFile(file);
    const result = await T.recognize(blob, "por+eng", {
      logger: m => { if (onProg && m.status) onProg(m.status, m.progress || 0); }
    });
  ...
  window.onEscribaPic = function (inp) {
    ...
    if (box) {
      ...
      p.className = "muted";
      p.id = "ocr-status";
      p.textContent = "OCR Tesseract local · por+eng. A primeira vez baixa o motor (~alguns MB).";
      box.appendChild(p);
    }
  };
  ...
  window.ocrEscriba = async function (file) {
    ...
    try {
      ...
      const out = await window.runLocalOcr(file, (st, pr) => {
        if (status) status.textContent = st + (pr ? " " + Math.round(pr * 100) + "%" : "");
      });
      if (q && out.query) q.value = out.query;
      if (status) {
        status.textContent = out.text
          ? ("Lido (" + Math.round(out.conf) + "%): " + out.text.replace(/\n/g, " · "))
          : "Nenhum texto nítido. Aproxime o rótulo ou escreva o nome.";
      }
      if (out.query && out.conf >= 40 && typeof window.runEscriba === "function") {
        window.runEscriba();
      }
    } catch (err) {
      if (status) status.textContent = "OCR falhou: " + (err.message || err) + ". Rede precisa baixar o motor uma vez.";
    }
  };
})();
